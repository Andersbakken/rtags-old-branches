...
IndexerJob::IndexerJob(const SharedPtr<Indexer> &indexer, unsigned flags, const Path &p, const List<ByteArray> &arguments)
...
IndexerJob::IndexerJob(const QueryMessage &msg, const SharedPtr<Project> &project,
    ...
    if (!clang_equalLocations(location, nullLocation)) {
        ...
        if (file) {
            ...
            if (blocked) {
                ...
                if (state == Unset) {
                    SharedPtr<Indexer> indexer = mIndexer.lock();
                    SharedPtr<IndexerJob> job = std::static_pointer_cast<IndexerJob>(shared_from_this());
...
void IndexerJob::execute()
{
    ...
    if (mDump) {
        assert(id() != -1);
        if (SharedPtr<Project> p = project()) {
            parse();
            if (mUnit) {
    ...
    if (SharedPtr<Indexer> idx = indexer()) {
        SharedPtr<IndexerJob> job = std::static_pointer_cast<IndexerJob>(shared_from_this());
