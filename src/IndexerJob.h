...
public:
    ...
    IndexerJob(const SharedPtr<Indexer> &indexer, unsigned flags,
               const Path &input, const List<ByteArray> &arguments);
    IndexerJob(const QueryMessage &msg, const SharedPtr<Project> &project,
               const Path &input, const List<ByteArray> &arguments);

    int priority() const { return mFlags & Priorities; }
    SharedPtr<IndexData> data() const { return mData; }
    ...
    bool abortIfStarted();
    SharedPtr<Indexer> indexer() { MutexLocker lock(&mMutex); return mIndexer.lock(); }
