...
ReferencesJob::ReferencesJob(const Location &loc, const QueryMessage &query, const SharedPtr<Project> &proj)
    : Job(query, 0, proj)
{
    locations.insert(loc);
}

ReferencesJob::ReferencesJob(const ByteArray &sym, const QueryMessage &query, const SharedPtr<Project> &proj)
