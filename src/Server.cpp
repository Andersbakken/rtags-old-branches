...
void Server::clear()
{
    ...
    mServer = 0;
    mProjects.clear();
    setCurrentProject(SharedPtr<Project>());
}
...
{
    SharedPtr<Project> &project = mProjects[dir];
    if (!project)
...
{
    SharedPtr<Project> project = mProjects.value(path);
    if (project) {
...
void Server::onMakefileParserDone(MakefileParser *parser)
{
    ...
    SharedPtr<Project> project = mProjects.value(parser->makefile());
    if (connection) {
...
void Server::handleCreateOutputMessage(CreateOutputMessage *message, Connection *conn)
{
    ...
    if (message->level() == CompilationError) {
        SharedPtr<Project> project = currentProject();
...
void Server::followLocation(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<FollowLocationJob> job(new FollowLocationJob(loc, query, project));
    job->setId(nextId());
...
void Server::findFile(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<FindFileJob> job(new FindFileJob(query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::dumpFile(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<IndexerJob> job(new IndexerJob(query, project, c.sourceFile, c.args));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::cursorInfo(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<CursorInfoJob> job(new CursorInfoJob(loc, query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::referencesForLocation(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<ReferencesJob> job(new ReferencesJob(loc, query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::referencesForName(const QueryMessage& query, Connection *conn)
{
    ...
    SharedPtr<ReferencesJob> job(new ReferencesJob(name, query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::findSymbols(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<FindSymbolsJob> job(new FindSymbolsJob(query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::listSymbols(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<ListSymbolsJob> job(new ListSymbolsJob(query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::status(const QueryMessage &query, Connection *conn)
{
    ...
    SharedPtr<StatusJob> job(new StatusJob(query, project));
    job->setId(nextId());
    mPendingLookups[job->id()] = conn;
    startJob(job);
}
...
void Server::isIndexed(const QueryMessage &query, Connection *conn)
{
    ...
    if (path.isFile()) {
        ...
        if (fileId) {
            SharedPtr<Project> old = mCurrentProject.lock();
            updateProjectForLocation(path);
            SharedPtr<Project> cur = currentProject();
...
void Server::startJob(const SharedPtr<Job> &job)
{
...
bool Server::processSourceFile(const GccArguments &args, const Path &proj)
{
    ...
    SharedPtr<Project> &project = mProjects[proj];
    if (!project) {
...
void Server::event(const Event *event)
{
    ...
    case JobOutputEvent::Type: {
        ...
        if (it == mPendingLookups.end()) {
            if (SharedPtr<Job> job = e->job.lock())
                job->abort();
            break;
        }
...
SharedPtr<Project> Server::setCurrentProject(const Path &path)
{
...
{
    SharedPtr<Project> match;
    int longest = -1;
    SharedPtr<Project> cur = mCurrentProject.lock();
...
SharedPtr<Project> Server::setCurrentProject(const SharedPtr<Project> &proj)
{
    SharedPtr<Project> old = mCurrentProject.lock();
...
bool Server::smartProject(const Path &path, const List<ByteArray> &extraCompilerFlags)
{
    ...
    if (dirs.isEmpty())
        return false;
    SharedPtr<Project> &project = mProjects[path];
...
void Server::project(const QueryMessage &query, Connection *conn)
{
    if (query.query().isEmpty()) {
        SharedPtr<Project> current = currentProject();
        for (ProjectsMap::const_iterator it = mProjects.begin(); it != mProjects.end(); ++it) {
            conn->write<128>("%s%s",
                             it->first.constData(),
                             it->second == current ? " <=" : "");
...
void Server::save(const SharedPtr<Indexer> &indexer)
{
...
void Server::onJobsComplete(SharedPtr<Indexer> indexer, int count)
{
    bool ok;
    ...
    if (count) {
        ...
        mSaveTimers[indexer] = EventLoop::instance()->addTimer(SaveTimerInterval, Server::saveTimerCallback,
                                                               new SharedPtr<Indexer>(indexer));
    }
}

void Server::saveTimerCallback(int id, void *userData)
{
    SharedPtr<Indexer> *indexer = static_cast<SharedPtr<Indexer> *>(userData);
    EventLoop::instance()->removeTimer(id);
...
void Server::onJobStarted(SharedPtr<Indexer> indexer, Path path)
{
