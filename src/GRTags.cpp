...
void GRTags::init(const SharedPtr<Project> &project)
{
    assert(project);
    mProject = project;
    recurse();
...
void GRTags::recurse()
{
    ...
    Server::instance()->threadPool()->start(SharedPtr<ThreadPool::Job>(job));
}
...
void GRTags::add(const Path &source)
{
    ...
    SharedPtr<GRParseJob> job(new GRParseJob(source, flags, project));
    ++mActive;
    ++mCount;
    job->finished().connect(this, &GRTags::onParseJobFinished);
    Server::instance()->threadPool()->start(job);
}
void GRTags::onParseJobFinished(const SharedPtr<GRParseJob> &job, const GRMap &data)
{
