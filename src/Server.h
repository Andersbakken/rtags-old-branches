...
public:
    ...
    void startJob(const SharedPtr<Job> &job);
    struct Options {
...
private:
    void onJobsComplete(SharedPtr<Indexer> indexer, int count);
    void onJobStarted(SharedPtr<Indexer> indexer, Path path);

    static void saveTimerCallback(int id, void *userData);
    void save(const SharedPtr<Indexer> &indexer);
    void restore();
    void clear();
    void onNewConnection();
    signalslot::Signal2<int, const List<ByteArray> &> &complete() { return mComplete; }
    SharedPtr<Project> setCurrentProject(const Path &path);
    SharedPtr<Project> setCurrentProject(const SharedPtr<Project> &proj);
    ...
    SharedPtr<Project> currentProject() const { return mCurrentProject.lock(); }
    ...
    WeakPtr<Project> mCurrentProject;
    ThreadPool *mThreadPool;
    signalslot::Signal2<int, const List<ByteArray> &> mComplete;
    Path mClangPath;

    Map<SharedPtr<Indexer>, int> mSaveTimers;
